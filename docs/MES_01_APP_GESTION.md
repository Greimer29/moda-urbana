...
- Roles base: **`ADMIN`** (acceso total) y **`OPERATOR`** (acceso según permisos asignados).
- Catálogo fijo de permisos por módulo (`ventas.confirm`, `catalog.edit`, etc.) almacenados en `users.permissions` (JSON) para operadores.
- Middleware de auth en rutas `/api/v1` + middleware de permisos que valida capacidades por ruta.
- Panel **Usuarios** (`/users`) para CRUD y matriz de permisos (solo quien tenga `users.manage`).
- Presets de perfil en UI (Vendedor, Inventario, Contador) sin tabla de roles en BD.
...
- `POST /auth/logout`
- `GET /auth/me` — datos del usuario actual + permisos efectivos

### Usuarios

- `GET /users` — listado paginado (`users.view`)
- `GET /users/:id` — detalle (`users.view`)
- `POST /users` — crear (`users.manage`)
- `PUT /users/:id` — actualizar (`users.manage`)
- `PATCH /users/:id/active` — activar/desactivar (`users.manage`)
...
3. **Clientes**
   ...
   - Estado de cuenta (`/customers/:id/cuenta`) para clientes con crédito.
4. **Usuarios**
   - Listado con búsqueda.
   - Modal crear/editar con matriz de permisos y presets (Vendedor, Inventario, Contador).
   - Activar/desactivar usuarios (`users.manage`).
5. **Proveedores**
   - Listado con búsqueda.
