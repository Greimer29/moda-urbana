...
export async function listSalesShifts(params: { page?: number; per_page?: number; status?: 'OPEN' | 'CLOSED' } = {}) {
...
export async function openSalesShift(notes?: string) {
  const { data } = await api.post<ShiftMutationResponse>('/sales-shifts/open', notes ? { notes } : {})
  return data.data.sales_shift
}
